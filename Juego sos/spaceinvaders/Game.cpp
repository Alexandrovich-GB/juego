...
void Game::initVar()
{
    this->window = nullptr;               // Inicializa el puntero de la ventana a nullptr 
...
void Game::initWind()
{
    ...
    this->window = new sf::RenderWindow(this->videoMode, "Space inaders"); // Crea la ventana del juego con el título especificado
}

Game::Game()
{
    this->initVar();    // Inicializa variables y objetos
    this->initWind();  
    level = 1;        
    gameOver = 0;       
...
Game::~Game()
{
    delete this->window;           // Libera la memoria de la ventana
    delete this->enemiesManager;   // Libera la memoria del gestor de enemigos
}
...
void Game::pollEvents()
{
    ...
    while (this->window->pollEvent(this->ev))
    {
        switch (this->ev.type)
        {
        case sf::Event::Closed:    // Cierre de ventana 
            this->window->close(); 
            break;
...
void Game::displayScore()
{
    ...
    text.setString("Puntos: " + std::to_string(ship.returnScore()));  
